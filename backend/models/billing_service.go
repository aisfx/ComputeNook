...
// GetRechargeRecordsV2 获取充值记录（新版本）
func GetRechargeRecordsV2(qosName string, limit int) ([]RechargeRecord, error) {
	query := `SELECT id, qos_name, amount, operator, notes, created_at 
	          FROM recharge_records WHERE qos_name = ? 
	          ORDER BY created_at DESC LIMIT ?`
	rows, err := DB.Query(query, qosName, limit)
	if err != nil {
