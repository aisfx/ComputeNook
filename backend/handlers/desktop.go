...
func buildDesktopScript(session *DesktopSession) string {
	...
	b.WriteString("  _pid=$(cat \"$lf\" 2>/dev/null | tr -d ' \\n')\n")
	b.WriteString("  [ -z \"$_pid\" ] && { rm -f \"$lf\"; continue; }\n")
	b.WriteString("  kill -0 \"$_pid\" 2>/dev/null || { _d=$(basename \"$lf\" | grep -oE '[0-9]+'); rm -f \"$lf\" /tmp/.X11-unix/X${_d} 2>/dev/null; }\n")
	b.WriteString("done\n\n")
