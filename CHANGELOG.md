...
- **改为单个下拉框显示所有镜像**（2026-07-03 21:51）
  ...
  - 下拉框支持搜索过滤（输入关键词即可筛选）
  - 首次打开下拉框时自动加载所有镜像
  - 镜像显示格式：`项目/仓库:标签`（如：`jupyter/base-notebook:x86_64-python-3.11.6`）
  - 选择后自动填入完整镜像路径（含Harbor域名）
  - 简化操作：从3步选择或点击弹框改为1步选择
  - 修改文件：`frontend/src/pages/user/jobs/index.tsx`
  - 提交哈希：`752fc088`

- **添加镜像加载调试日志**（2026-07-03 21:45）
  - 容器镜像改为输入框+选择按钮的UI方式
  - 点击"选择"按钮弹出Modal显示所有可用镜像
  - Modal中展示完整镜像列表：
    - 加载所有项目的所有仓库的所有标签
    - 每个镜像显示：项目/仓库:标签
    - 显示完整镜像路径（含Harbor域名）
...
- **容器镜像选择UI重构**（2026-07-03 21:16）
  ...
  - 修复缺失的组件导入：
    - 添加 `List` 组件
    - 添加 `Spin` 组件
    - 添加 `DatabaseOutlined` 图标
  - 优化用户体验，简化镜像选择流程
  - 修改文件：`frontend/src/pages/user/jobs/index.tsx`
  - 提交哈希：`f53cf279`

### Added
- **容器作业添加挂载目录和工作目录字段**（2026-07-03 21:10）
  - 容器作业新增"挂载目录"字段
    - 格式：宿主机路径:容器内路径
    - 支持多个挂载，逗号分隔
    - 默认示例：/fs/home/username:/fs/home/username
  - 容器作业新增"工作目录"字段
    - 指定容器内的工作目录
    - 默认示例：/fs/home/username
  - 运行命令改为可选
    - 留空 = 交互模式（sleep infinity）
    - 可通过"进入容器"连接
  - 添加提示信息，说明交互模式用法
  - 修改文件：`frontend/src/pages/user/jobs/index.tsx`

- **新增镜像仓库管理页面**（2026-07-03 21:00）
  - 新增用户侧"镜像仓库"菜单和页面
  - 左右分栏布局：左侧项目列表，右侧仓库详情
  - 左侧显示所有项目（公开/私有图标和标签）
  - 右侧显示选中项目的仓库列表
  - 每个仓库显示：名称、镜像数、更新时间、镜像地址
  - 一键复制镜像地址功能
  - 使用说明按钮，指导用户如何使用镜像
  - 支持搜索仓库
  - 新增文件：
    - `frontend/src/pages/user/registry/index.tsx`
  - 修改文件：
    - `frontend/src/layouts/UserLayout.tsx`

### Changed
- **移除作业日志AI分析按钮**（2026-07-03 20:55）
  - 移除作业日志弹窗中的"AI 分析问题"按钮
  - 用户可以通过页面右下角的小猴子AI助手来分析日志内容
  - 避免功能重复，界面更简洁
  - 删除相关代码：
    - `aiAnalyzing` 状态
    - `analyzeJobLog` 函数
    - Modal footer中的AI分析按钮
  - 保留"下载日志"功能
  - 修改文件：`frontend/src/pages/user/jobs/index.tsx`

### Fixed
- **作业提交表单脚本编辑问题修复**（2026-07-03 20:50）
  - 修复脚本内容无法手动编辑的问题
  - 问题原因：`onValuesChange`监听器在任何字段改变时都会重新生成脚本，覆盖用户的手动编辑
  - 解决方案：
    - 移除表单级别的`onValuesChange`自动更新
    - 移除所有字段的`onChange={updateScriptFromForm}`
    - 添加"生成脚本模板"按钮，用户可以主动触发脚本生成
    - 用户可以使用生成的模板，也可以完全手动编辑
  - 工作目录输入框优化：
    - 添加白色背景样式，确保可见可编辑
    - 修改placeholder为更清晰的示例格式
    - 将"选择"按钮改为"提示"，引导用户直接输入
  - 脚本输入框优化：
    - 添加白色背景样式
    - 添加tooltip说明可以生成或手动编辑
    - 修改placeholder提示点击按钮生成或直接输入
  - 修改文件：`frontend/src/pages/user/jobs/index.tsx`
