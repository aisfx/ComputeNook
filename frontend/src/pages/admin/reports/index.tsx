...
export default function AdminReportsPage() {
  ...
  const [selectedUser, setSelectedUser] = useState<string>('') // 空表示查询所有用户
  ...
  const handleQuery = async () => {
    ...
    const params = {
      ...
      user: selectedUser || undefined, // 管理员可以指定用户,空表示查询所有
    }
          ...
          <Space wrap>
            ...
            <Select
              ...
              options={[
                { label: '全部用户', value: '' },
                ...users.map((u) => ({
                  label: u.cnName ? `${u.cnName} (${u.username})` : u.username,
                  value: u.username,
                })),
              ]}
            />
  ...
  async function exportExcel() {
    ...
    try {
      ...
      const userInfo = selectedUser ? `用户: ${selectedUser}` : '全部用户'

      // 月度作业趋势
      if (jobStats) {
        const j = jobStats
        const ws1 = XLSX.utils.aoa_to_sheet([
          ['统计范围', `${startDate} ~ ${endDate}`, userInfo],
          [],
          ['月份', '队列', '作业数'],
      ...
      const fileName = selectedUser 
        ? `报表中心_${selectedUser}_${startDate}_${endDate}.xlsx`
        : `报表中心_全部用户_${startDate}_${endDate}.xlsx`
