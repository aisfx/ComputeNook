...
if (__VLS_ctx.showModal) {
    ...
    (__VLS_ctx.isEditing ? 'Account' : 'Account (OpenLDAP 用户组)');
    if (!__VLS_ctx.isEditing) {
        ...
        for (const [group] of __VLS_vFor((__VLS_ctx.ldapGroups))) {
            ...
            (group.gid);
            // @ts-ignore
            [showModal, isEditing, isEditing, isEditing, isEditing, closeModal, formData, ldapGroups,];
        }
    }
    else {
    ...
    (__VLS_ctx.isEditing ? 'Slurm Account 名称不可修改' : '选择一个 LDAP 用户组作为 Slurm Account 名称');
    __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
        ...{ class: "form-group" },
    });
    /** @type {__VLS_StyleScopedClasses['form-group']} */ ;
...
// @ts-ignore
[isEditing, closeModal, formData, formData, formData, saveAccount, saving, saving,];
var __VLS_3;
// @ts-ignore
[];
