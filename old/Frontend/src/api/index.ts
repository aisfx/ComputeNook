...
export const qosAPI = {
  ...
  // 充值机时
  rechargeQoS: async (qosName: string, amount: number, notes?: string) => {
  ...
  getRechargeHistory: async (qosName?: string, limit?: number) => {
    const params: any = {}
    if (qosName) params.qos_name = qosName
...
export const getAssociation = async (account: string, user: string, cluster: string) => {
...
export const createAssociation = async (association: any) => {
  return await axios.post('/slurm/associations', association)
}

export const updateAssociation = async (account: string, user: string, cluster: string, association: any) => {
  const params = new URLSearchParams()
  params.append('account', account)
  params.append('user', user)
  if (cluster) {
...
export const deleteAssociation = async (account: string, user: string, cluster: string, partition?: string) => {
  const params = new URLSearchParams()
  params.append('account', account)
  params.append('user', user)
  if (cluster) {
...
export const desktopAPI = {
  ...
  startSession: async (id: number, partition?: string) => {
