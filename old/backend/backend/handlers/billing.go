...
func GetBillingAccounts(c *gin.Context) {
	...
	if err == nil {
		...
		for _, account := range accounts {
			...
			totalMins := int64(account.TotalRecharged * 60)
			usedMins := int64(actualUsed * 60)
			result = append(result, map[string]interface{}{
				...
				"used_mins":   usedMins,
				"remain_mins": totalMins - usedMins,
...
// GetRechargeRecords 获取充值记录（qos_name 可选，不传则返回全部）
func GetRechargeRecords(c *gin.Context) {
	...
	records, err := models.GetRechargeRecordsV2(qosName, limit)
	if err != nil {
